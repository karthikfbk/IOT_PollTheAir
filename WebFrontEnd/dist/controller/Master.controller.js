...
sap.ui.define([
		...
		return BaseController.extend("poll.the.air.controller.Master", {
			...
			onSort : function (oEvent) {
				var sKey = oEvent.getSource().getSelectedItem().getKey(),
					aSorters = this._oGroupSortState.sort(sKey,true);
			...
			onGroup : function (oEvent) {
				var sKey = oEvent.getSource().getSelectedItem().getKey(),
					aSorters = this._oGroupSortState.group(sKey,true);
			...
			onOpenViewSettings : function () {
				if (!this._oViewSettingsDialog) {
					this._oViewSettingsDialog = sap.ui.xmlfragment("poll.the.air.view.ViewSettingsDialog", this);
			...
			onConfirmViewSettingsDialog : function (oEvent) {
				...
				aFilterItems.forEach(function (oItem) {
					switch (oItem.getKey()) {
						case "Filter1" :
							aFilters.push(new Filter("Latitude", FilterOperator.LE, 100));
							break;
						case "Filter2" :
							aFilters.push(new Filter("Latitude", FilterOperator.GT, 100));
							break;
						default :
							break;
					}
			...
			_onMasterMatched :  function() {
				this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
					function (mParams) {
						...
						var slat = mParams.firstListitem.getBindingContext().getProperty("Latitude");
						var slong = mParams.firstListitem.getBindingContext().getProperty("Longitude");
						var spol = mParams.firstListitem.getBindingContext().getProperty("Pollution");
						this.getRouter().navTo("object", {objectId : sObjectId, lati: slat, longi : slong, polli : spol}, true);
					}.bind(this),
					function (mParams) {
						if (mParams.error) {
							return;
			...
			_showDetail : function (oItem) {
				...
				this.getRouter().navTo("object", {
					...
					lati : oItem.getBindingContext().getProperty("Latitude"),
					longi : oItem.getBindingContext().getProperty("Longitude"),
					polli : oItem.getBindingContext().getProperty("Pollution")
				}, bReplace);
			},
